    ...
    async function loadCases(){ const list=await getJSON('/v1/investigations?limit=50'); const el=document.getElementById('casesList'); if(!list||list.__error||!list.length){ el.innerHTML='<div class="empty">'+t('cases_none')+'</div>'; document.getElementById('navCases').textContent='0'; return; } document.getElementById('navCases').textContent=list.length; el.innerHTML=list.map(c=>`<div class="case-card" data-action="open-inv-workspace" data-id="${escapeHtml(c.id)}"><div class="ct">${escapeHtml(c.title||'(untitled)')} <i class="ri-arrow-right-line" style="color:var(--accent)"></i></div><div class="cm">seed: ${escapeHtml(c.seed_source||'?')}/${escapeHtml(c.seed_dataset||'?')} · ${escapeHtml(c.status||(c.steps&&c.steps.length?c.steps.length+' steps':'empty'))} · ${escapeHtml(relTime(c.updated_at||c.created_at))}</div></div>`).join(''); }
