    ...
    function updateAgentStrip(insightCount, health, since){
      ...
      if(failing.length){ msg.innerHTML=t('agent_failing_open',{src:escapeHtml(failing.map(f=>sourceLabel(f.source)).join(', '))}); }
    ...
    async function dvInit(){
      ...
      const seen={};
      for(const sid of ['1','2']){ const sel=document.getElementById('dvSrc'+sid); sel.innerHTML=''; for(const s of srcs){ if(!seen[s.source+sid]){ seen[s.source+sid]=true; const o=document.createElement('option'); o.value=s.source; o.textContent=sourceLabel(s.source); sel.appendChild(o); } } }
    ...
    function insightCard(i, withActions){
      ...
      return `<div class="card sev-${i.severity} ${unread?'unread':''}" id="card-${cssEsc(i.id)}" data-kind="${escapeHtml(i.kind)}">
        ...
        <div class="meta"><span>${escapeHtml(sourceLabel(i.source))}/${escapeHtml(i.dataset)}</span><span>${escapeHtml(i.kind)}</span><span>${t('card_conf',{n:Math.round((i.confidence||0)*100)})}</span><span>${t('card_by',{who:escapeHtml(i.producer)})}</span></div>
    ...
    function facetValueLabel(facet,value){
      ...
      if(key) return t(key);
      // The `source` facet value is "source/dataset" — translate the source
      // slug via sourceLabel() so e.g. "midland/midland-bank-listings" renders
      // as "Midland / midland-bank-listings" (or 美聯物業 in zh-HK).
      if(facet==='source' && value){
        const slash=value.indexOf('/');
        if(slash>0) return sourceLabel(value.slice(0,slash))+' / '+value.slice(slash+1);
        return sourceLabel(value);
      }
      // The `field` facet value is a record field key — prettyField() humanizes it.
      if(facet==='field') return prettyField(value);
      return value;
    }
    ...
    async function degradedEmpty(message){ const hs=cachedHealth||await loadHealthQuiet(); const failing=(hs||[]).filter(s=>s.circuit&&s.circuit!=='closed'); if(failing.length){ return `<div class="empty">${escapeHtml(message)} <span style="color:var(--warn)">— ${t('degraded_upstream',{srcs:failing.map(f=>escapeHtml(sourceLabel(f.source))).join(', ')})} <i class="ri-refresh-line"></i> ${t('degraded_retry')}</span></div>`; } // If health is empty the API itself is unreachable (e.g. the public static
    ...
    // ============ signals ============
    async function sigSourceFill(){ const sel=document.getElementById('sigSource'); if(sel.options.length) return; const srcs=await getJSON('/v1/sources'); if(!srcs||srcs.__error) return; const seen={}; for(const s of srcs) if(!seen[s.source]){ seen[s.source]=true; const o=document.createElement('option'); o.value=s.source; o.textContent=sourceLabel(s.source); sel.appendChild(o); } sel.value='hkma'; sigDatasetFill(); }
    ...
    async function previewSignal(){ const box=document.getElementById('sigPreview'); box.textContent=t('sig_previewing'); const compiled=buildScanTarget(); const p=await postJSON('/v1/signals/preview',{compiled, window_days:90}); if(!p||p.__error){ box.innerHTML='<span style="color:var(--crit)">'+t('sig_preview_err',{detail:escapeHtml((p&&p.__text)||('HTTP '+(p&&p.__error)))})+'</span>'; return; } const n=p.count==null?(p.findings||[]).length:p.count; // D-032: when the records cache was cold the detector saw nothing — say so
      ...
      box.innerHTML=t('sig_fired',{n,days:(p.window_days||90),s:(n===1?'':'s'),det:escapeHtml(p.compiled.detector),src:escapeHtml(sourceLabel(p.compiled.source))+'/'+escapeHtml(p.compiled.dataset)})+'<br>'+(n?'<span style="color:var(--muted)">'+t('sig_recent',{list:((p.findings||[]).slice(0,3).map(f=>escapeHtml(f.record_id||f.title||'?')).join(', '))})+'</span>':'<span style="color:var(--ok)">'+t('sig_none_fired')+'</span>'); }
