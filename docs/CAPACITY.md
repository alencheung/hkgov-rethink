...
| **v2** | shared **Redis** cluster (`--features redis`) | cache hits across nodes | **implemented, NOT wired** — `RedisStore` exists behind the feature flag, but `store.backend=redis` produces a loud startup error in `main.rs` (`build_store` reads the config but only `memory` is instantiated). Also has an architectural issue: it serializes the whole dataset as a single blob rather than keying per-record. |
