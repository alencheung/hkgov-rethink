...
impl AlertDispatcher {
    ...
    pub fn from_settings(settings: &AlertSettings) -> Option<Self> {
        ...
        // Email sink (v9): build one if all four email fields are present.
        if let (Some(api_url), Some(token), Some(to), Some(from)) = (
            ...
            #[cfg(not(feature = "alerts"))]
            {
                let _ = (api_url, token, to, from);
                tracing::warn!(
                    "email alerting configured but `alerts` feature is off; emails will not send"
                );
            }
        }
        Some(Self {
...
// ---------------------------------------------------------------------------
// EmailSink (behind the `alerts` feature)
// ---------------------------------------------------------------------------

/// Sends each insight as an email via an HTTP email-API gateway (SendGrid /
/// Mailgun / SES-via-HTTP style). This is the realistic push channel —
/// webhook integrations require the recipient to run a server; email is
/// universal. Behind the `alerts` feature so the default build needs no HTTP.
///
/// The gateway contract is intentionally generic: a JSON POST to `api_url`
/// with `Authorization: Bearer <token>` and a body `{to, subject, text}`.
/// Most transactional-email APIs accept this shape directly or with a thin
/// adapter. SMTP-sending would need a heavy crate; HTTP-API-sending reuses
/// the existing reqwest client.
#[cfg(feature = "alerts")]
...
impl AlertSink for EmailSink {
    ...
    async fn dispatch(&self, insight: &Insight) -> Result<()> {
        let subject = format!("[{}] {}", insight.severity, insight.title);
        // PR-005 (Marcus A6): an alert with no evidence rows is not
        // self-justifying — a desk operator must be able to act from the
        // notification alone. Append a bounded slice of the insight's evidence
        // refs (record_id · field · value) so the alert carries its proof.
        const MAX_EVIDENCE_ROWS: usize = 5;
        let mut evidence_block = String::new();
        if !insight.evidence.is_empty() {
            evidence_block.push_str("\n\n--- evidence ---\n");
            for ev in insight.evidence.iter().take(MAX_EVIDENCE_ROWS) {
                let ctx = ev
                    .context
                    .as_deref()
                    .map(|c| format!(" ({c})"))
                ...
                evidence_block.push_str(&format!(
                    "{}{}: {} = {}\n",
                    ev.record_id, ctx, ev.field, ev.value
                ));
            }
            let hidden = insight.evidence.len().saturating_sub(MAX_EVIDENCE_ROWS);
            if hidden > 0 {
                evidence_block.push_str(&format!("… and {hidden} more (see dashboard)\n"));
            }
        }
        let body = serde_json::json!({
            ...
            "subject": subject,
            "text": format!(
                "{}\n\n{}\n\nseverity: {}\nconfidence: {:.0}%\nsource: {}/{}\nproduced by: {}{}\n\n---\nThis alert was generated by hkgov-rethink.",
                insight.title, insight.summary, insight.severity,
                insight.confidence * 100.0,
                insight.source, insight.dataset, insight.producer,
                evidence_block
            ),
        });
        ...
        let resp = self
            ...
            .header(
                ...
                reqwest::header::HeaderValue::from_str(&format!("Bearer {}", self.token)).map_err(
                    |e| hkgov_common::Error::Internal(format!("email auth header: {e}")),
                )?,
            )
            .json(&body)
            .send()
