...
| F-027 | Anti-abuse rate limit on `/ask` | The Nth+1 `POST /v1/ask` from one client (per_window, default 5/min) → `429` + `Retry-After` + `X-RateLimit-Remaining: 0`. Metered on three independent dimensions (session / device / IP); the first to hit its cap blocks. (`ratelimit.rs`, `routes.rs:rate_limit_guard`) | POST `/v1/ask` 6× within 60s from one IP | — | ✅ pass — 4th→429 w/ Retry-After; GETs unaffected (`ask_allows_under_cap_then_blocks`, `get_routes_are_unmetered`) |
| F-028 | Rate-limit warn threshold | At `ask_warn_at` (default 4) requests in the window the response still succeeds (200) but carries `X-RateLimit-Warning` + `X-RateLimit-Remaining` so cooperative clients slow down before being blocked. (`routes.rs:rate_limit_guard`) | POST `/v1/ask` 4× → 4th response has the warning header | — | ✅ pass (`ask_warns_at_threshold_before_blocking`) |
| F-029 | Per-IP backstop vs rotation | A caller cycling sessions/devices from one IP still hits the per-IP cap. New session, same IP, exhausted bucket → 429. (`ratelimit.rs`, `identity.rs`) | 3 asks w/ 3 different `Authorization: Bearer`, then a 4th new session | — | ✅ pass (`per_ip_backstop_catches_session_rotation`) |
| F-030 | Auth routes exempt from limiter | `POST /v1/auth/request-token` + `/auth/redeem` are NOT metered (magic-link spam is a separate concern). Hammering them never yields 429. (`routes.rs:expensive_writes`) | POST `/v1/auth/request-token` 20× rapidly | — | ✅ pass (`auth_routes_exempt_from_limiter`) |
| F-031 | Trusted-proxy XFF resolution | With `rate_trusted_proxies=N`, the per-IP dimension resolves the real client from `X-Forwarded-For` (rightmost-N-trusted-hop semantics); with 0, the TCP peer is used directly. (`identity.rs:client_ip`) | set `rate_trusted_proxies=1`, send `X-Forwarded-For` | — | ✅ pass (identity unit tests: xff_when_proxies_trusted, picks_last_untrusted_hop, too_short_falls_back_to_peer) |
| F-032 | Rate limit disabled at 0 | `ask_per_window=0` → no limit at all (documented "0 = unlimited" contract). (`routes.rs:rate_limit_guard`) | set `ask_per_window=0`, POST `/v1/ask` 30× | — | ✅ pass (`disabled_when_per_window_zero`) |
